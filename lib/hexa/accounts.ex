defmodule Hexa.Accounts do
  import Ecto.Query
  import Ecto.Changeset

  alias Hexa.Repo
  alias Hexa.Accounts.{User, Identity, Events}
  ...
  def admin?(%User{} = user) do
    user.email in Hexa.config([:admin_emails])
  end
  ...
  def update_public_settings(%User{} = user, attrs) do
    ...
    |> case do
      {:ok, new_user} ->
        Hexa.execute(__MODULE__, %Events.PublicSettingsChanged{user: new_user})
        {:ok, new_user}
