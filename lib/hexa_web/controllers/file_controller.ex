defmodule HexaWeb.FileController do
  @moduledoc """
  ...
  defp proxy_file(conn, ip, mime_type, content_length) do
    ...
    port = HexaWeb.Endpoint.config(:http)[:port]
  ...
  defp server_ip, do: Hexa.config([:files, :server_ip])

  defp file_server_opts do
    [
      hostname: Hexa.config([:files, :hostname]) || "localhost",
      transport_opts: Hexa.config([:files, :transport_opts]) || []
    ]
  end
end
