defmodule HexaWeb.ProfileLive do
  use HexaWeb, :live_view

  alias Hexa.{Accounts, MediaLibrary, MP3Stat}
  alias HexaWeb.{LayoutComponent, Presence}
  alias HexaWeb.ProfileLive.{SongRowComponent, UploadFormComponent}
  ...
  def mount(%{"profile_username" => profile_username}, _session, socket) do
    ...
    if connected?(socket) do
      ...
      HexaWeb.Presence.subscribe(profile)
    end
  ...
  def handle_info({Hexa.PresenceClient, %{user_joined: presence}}, socket) do
    {:noreply, assign_presence(socket, presence)}
  end

  def handle_info({Hexa.PresenceClient, %{user_left: presence}}, socket) do
    %{user: user} = presence
