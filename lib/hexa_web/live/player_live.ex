defmodule HexaWeb.PlayerLive do
  use HexaWeb, {:live_view, container: {:div, []}}

  alias Hexa.{Accounts, MediaLibrary}
  alias Hexa.MediaLibrary.Song

  on_mount {HexaWeb.UserAuth, :current_user}
  ...
  defp switch_profile(socket, nil) do
    ...
    if profile = connected?(socket) and socket.assigns.profile do
      Hexa.PresenceClient.untrack(profile, current_user.id)
    end
  ...
  defp switch_profile(socket, profile_user_id) do
    ...
    if profile && connected?(socket) do
      ...
      if socket.assigns.profile do
        Hexa.PresenceClient.untrack(socket.assigns.profile, current_user.id)
      end
      Hexa.PresenceClient.track(profile, current_user.id)
  ...
  def handle_info({MediaLibrary, %MediaLibrary.Events.PublicProfileUpdated{} = update}, socket) do
    ...
    if update.profile.user_id == socket.assigns.current_user.id do
      Hexa.PresenceClient.untrack(socket.assigns.profile, current_user.id)
      Hexa.PresenceClient.track(update.profile, current_user.id)
    end
