defmodule HexaWeb.LayoutView do
  use HexaWeb, :view
  ...
  def sidebar_nav_links(assigns) do
    ...
    <div class="space-y-1">
      <%= if @current_user do %>
        ...
        <.link
          navigate={Routes.ar_path(Endpoint, :index)}
          class={"text-gray-700 hover:text-gray-900 group flex items-center px-2 py-2 text-sm font-medium rounded-md #{if @active_tab == :ar, do: "bg-gray-200", else: "hover:bg-gray-50"}"}
          aria-current={if @active_tab == :ar, do: "true", else: "false"}
        >
          <.icon name={:camera} outlined class="text-gray-400 group-hover:text-gray-500 mr-3 flex-shrink-0 h-6 w-6"/>
          AR
        </.link>
